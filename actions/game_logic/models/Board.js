...
class Board {
    ...
    if (SquareUtils.hasPiece(laserSquare)) {
      ...
      completeRoute.push(
        new LaserPath(
          ...
          direction,
          actionType,
          laserSquareLocation.serialize(),
        ).serialize(),
      ); // start from the player's laser piece.
      while (eventType !== LaserEventsEnum.END) {
        ...
        if (rowIndex < 0 || rowIndex > 7 || colIndex < 0 || colIndex > 9) {
          ...
          completeRoute.push(
            new LaserPath(
              ...
              null,
              actionType,
              new Location(colIndex, rowIndex).serialize(),
            ).serialize(),
          );
          continue;
        }
        ...
        completeRoute.push(
          new LaserPath(
            ...
            direction,
            actionType,
            nextScanningSquareLocation.serialize(),
          ).serialize(),
        );
      }
    }
  ...
  getMovesForPieceAtLocation(location) {
    ...
    possibilities.forEach(([dx, dy]) => {
      ...
      const movePossibility = this.checkMovePossibility(
        location,
        possibleDestLocation,
      );
  ...
  checkMovePossibility(srcLocation, destLocation) {
    ...
    if (
      squareAtDest.type === SquareTypesEnum.RESERVED_BLUE &&
      pieceColorAtSrc !== PlayerTypesEnum.BLUE
    ) {
      // Trying to make a movement to a square reserved for blue player pieces only,
      // with a red player's piece.
      return new Movement(MovementTypesEnum.INVALID, srcLocation, destLocation);
    } else if (
      squareAtDest.type === SquareTypesEnum.RESERVED_RED &&
      pieceColorAtSrc !== PlayerTypesEnum.RED
    ) {
    ...
    // Special Move (swap)
    if (
      pieceTypeAtSrc === PieceTypesEnum.SWITCH &&
      (pieceTypeAtDest === PieceTypesEnum.DEFENDER ||
        pieceTypeAtDest === PieceTypesEnum.DEFLECTOR)
    ) {
      ...
      if (SquareUtils.hasPiece(squareAtDest)) {
        ...
        // Invalid move
        return new Movement(
          MovementTypesEnum.INVALID,
          srcLocation,
          destLocation,
        );
      } else {
        return new Movement(
          MovementTypesEnum.NORMAL,
          srcLocation,
          destLocation,
        );
      }
  ...
  applyMovement(movement) {
    ...
    if (movement.type === MovementTypesEnum.NORMAL) {
      ...
      const squareAtDest =
        this.squares[movement.destLocation.rowIndex][
          movement.destLocation.colIndex
        ];
  ...
  applyLaser(playerType) {
    if (!playerType) {
      throw new Error(
        "applyLaser - Please specify the player whose laser is being switched on.",
      );
    }
    ...
    if (finalLaserPath.actionType === LaserActionTypesEnum.KILL) {
      const squareAtHit =
        this.squares[finalLaserPath.location.rowIndex][
          finalLaserPath.location.colIndex
        ];
      ...
      if (squareAtHit.piece.type === "k") {
        ...
        this.winner =
          squareAtHit.piece.color === PlayerTypesEnum.BLUE
            ? PlayerTypesEnum.RED
            : PlayerTypesEnum.BLUE;
      }
  ...
  static linePointsFromLaserRoute(laserRoute, cellSize) {
    const points = laserRoute.map((path) => {
      let x, y;
      if (
        path.eventType === LaserEventsEnum.START ||
        path.eventType === LaserEventsEnum.END
      ) {
